...
def text_at_xy( x, y, text):
    tur.penup()
    tur.goto(x, y)
    tur.write(text, font=("Arial", 40, "bold"))
...
def play(x,y):
    ...
    if result==1: text_at_xy(-4, -4.5, "X Wins :D, Boo O :("),screen.exitonclick() 
    elif result==2: text_at_xy(-4, -4.5, "O Wins:D, Boo X :("),screen.exitonclick() 
    elif result==3: text_at_xy(-1, -4.5, "TIE!!"),screen.exitonclick() 
